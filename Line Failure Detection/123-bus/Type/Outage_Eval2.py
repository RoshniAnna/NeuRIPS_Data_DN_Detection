...
def Outage_evaluate(DSSfile,Outlist, Multfact, G_original):
    ...
    u = randint(1,3)
    t = randint(0,3)
    if t == 0 and len(Outlist) != 0:
        Out_type = '3-Phase Outage'
    elif t>0 and u==t and len(Outlist) != 0:
        Out_type = '1-Phase Outage'
    elif t>0 and u!=t and len(Outlist) != 0:
        Out_type = '2-Phase Outage'
    else:
        Out_type = 'No Outage'
        
    for e in Outlist:
        #loc_d_obj.dssText.Command=('Edit '+ e['device'] + '.' +  e['label'] + ' enabled=False') #disable to component
        loc_d_obj.dssCircuit.SetActiveElement(e['device'] + '.' +  e['label'])
        loc_d_obj.dssCktElement.Open(1,t)
        loc_d_obj.dssCktElement.Open(1,u)
        loc_d_obj.dssSolution.Solve()
    ...
    return (Bus_Pserved,Bus_PDemand,Bus_Voltages,BranchFlow,Out_type)    