...
def write_batch():
    ...
    if Scenarios:
        out_name = f"{timestamp}_{run_id}_EVCSAttacks_123_job{job_index}_part{batch_count}.pkl.gz"
...
for _ in range(scenarios_per_job):
    print(f"Normal Scenario {scid}", flush =True)
...
for idx in range(scenarios_per_job):
