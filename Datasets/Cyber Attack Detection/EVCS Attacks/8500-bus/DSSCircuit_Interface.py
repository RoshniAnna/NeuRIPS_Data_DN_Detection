...
def CircuitSetup(DSSfile: str, StationsInfo) -> DSSCircuit:
    """Compiles and sets up a DSS circuit object."""
    dss_obj = DSSCircuit(DSSfile)
    dss_obj.compile_ckt()
    dss_obj.dss.Text.Command("Set Maxiterations=5000")
    dss_obj.dss.Text.Command("Set Maxcontroliter=5000")
    dss_obj.dss.Basic.AllowForms(0)
    
    # Storage Allocation 
    for ChargingStation in StationsInfo:
        dss_obj.dss.Text.Command("New Storage.ChargeStatn" + str(ChargingStation['no']) + " bus1=" + ChargingStation['bus'] + ChargingStation['phaseconn'] + " phases=" + str(ChargingStation['numphase']) + " kv=" + str(ChargingStation['kV']) + " kW=" + str(ChargingStation['kw'])+ " kWrated=" + str(ChargingStation['kw']) +
