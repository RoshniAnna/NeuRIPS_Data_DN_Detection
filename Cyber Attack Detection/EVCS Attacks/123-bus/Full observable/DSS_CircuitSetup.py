...
The objects for circuit, bus, and branch are also set up.
...
class Branch:  # to extract properties of branch
    def __init__(self, DSSCktobj, branch_fullname):
        ...
        power_flow = np.array(DSSCktobj.dss.CktElement.Powers())
        ctidx = 2 * np.array(range(0, min(int(i.size/ 4), 3)))
        I_mag = i[ctidx] #branch current in A
        branchflow =  power_flow[ctidx]
...
def CktModSetup(DSSfile): # give the charging station info as input
    ...
    DSSCktobj.dss.Text.Command("Set Maxiterations=100000")
    DSSCktobj.dss.Text.Command("Set maxcontroliter=100000")
    DSSCktobj.dss.Text.Command("Set ControlMode = OFF")
