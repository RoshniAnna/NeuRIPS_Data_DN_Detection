...
for idx in range(NSc_attack//2):
    ...
    for atk_bus in attack_buses:
        attacked_voltages = inject_voltage_attack(V_node_Sc[atk_bus], start_idx, end_idx, attack_mult) 
        V_node_Sc[atk_bus] = attacked_voltages
    Scenarios.append({'Index':scid, 'Anomalous':'Yes', 'Targeted Buses': attack_buses, 'Attack Type': 'Over Voltage', 'BusVoltage series':attacked_voltages,'BranchFlow series':flow_branch_Sc})
    scid = scid + 1
...
# Write list
with open(FolderName+ './SensorAttacks_34.pkl', 'wb') as file:
